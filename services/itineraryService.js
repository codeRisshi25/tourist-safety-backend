  ...
  function validateItineraryPayload(body) {
    const requiredRoot = ["tripName", "startDate", "endDate", "days"];
    ...
    body.days.forEach((dayObj, idx) => {
      if (typeof dayObj.day !== "number")
        throw new Error(`Day index ${idx} missing numeric 'day'`);
  ...
  export async function createItinerary(data) {
    ...
    const safetyScore = computeSafetyScore(geocodedLocations, riskZones);
    const newId = itineraries.length
      ? Math.max(...itineraries.map((i) => i.id || 0)) + 1
      : 1;
